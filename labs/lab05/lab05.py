...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    x0, y0 = get_lat(city_a), get_lon(city_a)
    x1, y1 = get_lat(city_b), get_lon(city_b)
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[x // 2] if x % 2 == 0 else deck[len(deck) // 2 + (x - 1) // 2] for x in range(len(deck))]
